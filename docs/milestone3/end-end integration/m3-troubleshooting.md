...
End-to-end guidance for running the Stylus-first **shim** workflow in continuous integration and during local development. Scope is limited to Milestone 3’s precompile shims and reseeding/validation flow (no VM precompile injection).
...
* Shims predeployed at the canonical addresses.
