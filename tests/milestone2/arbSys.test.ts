...
describe("ArbSys Precompile Handler", () => {
  ...
  describe("Basic ArbSys Methods", () => {
    it("should handle arbChainID() correctly", async () => {
      ...
      expect(decodedChainId).to.equal(42161);
    });
    ...
    it("should handle arbBlockNumber() correctly", async () => {
      ...
      expect(decodedBlockNumber).to.equal(12345);
    });
    ...
    it("should handle arbOSVersion() correctly", async () => {
      ...
      expect(decodedVersion).to.equal(20);
    });
  });
  ...
  describe("sendTxToL1 Functionality", () => {
    it("should handle sendTxToL1() correctly", async () => {
      ...
      // Set offset to data (should be 68)
      const offsetView = new DataView(calldata.buffer, 4);
      offsetView.setBigUint64(24, BigInt(68), false);
      
      // Set data length
      const lengthView = new DataView(calldata.buffer, 36);
      lengthView.setBigUint64(24, BigInt(4), false);
      
      // Set destination address (padded to 32 bytes)
      const addressBytes = destAddress.slice(2); // Remove 0x
      for (let i = 0; i < 20; i++) {
        calldata[48 + i] = parseInt(addressBytes.slice(i * 2, i * 2 + 2), 16);
      }
      
      // Set data
      const dataBytes = data.slice(2); // Remove 0x
      for (let i = 0; i < 4; i++) {
        calldata[68 + i] = parseInt(dataBytes.slice(i * 2, i * 2 + 2), 16);
      }
      ...
      expect(decodedMessageId).to.be.greaterThan(0);
    });
  ...
  describe("Address Aliasing Functionality", () => {
    it("should handle mapL1SenderContractAddressToL2Alias() correctly", async () => {
      ...
      expect(decodedL2Alias).to.equal(Number(expectedL2Alias));
    });
    ...
    it("should reject mapL1SenderContractAddressToL2Alias with invalid calldata length", async () => {
      ...
      expect(result.success).to.be.false;
      expect(result.error).to.include("Invalid calldata length for mapL1SenderContractAddressToL2Alias");
    });
  });
  ...
  describe("Integration with HardhatArbitrumPatch", () => {
    it("should be properly registered in the patch", () => {
      ...
      expect(arbSysHandler!.address).to.equal("0x0000000000000000000000000000000000000064");
    });
