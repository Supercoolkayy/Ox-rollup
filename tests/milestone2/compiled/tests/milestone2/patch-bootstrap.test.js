...
describe("Plugin Bootstrap", () => {
    ...
    describe("initArbitrumPatch", () => {
        it("should initialize plugin when enabled", () => {
            ...
            (0, hardhat_patch_1.initArbitrumPatch)(mockHre, { enable: true });
            // Check that the plugin was attached to the environment
            expect(mockHre.arbitrumPatch).to.be.instanceOf(hardhat_patch_1.HardhatArbitrumPatch);
            // Verify the registry contains the expected handlers
            const plugin = mockHre.arbitrumPatch;
        ...
        it("should not initialize plugin when disabled", () => {
            ...
            (0, hardhat_patch_1.initArbitrumPatch)(mockHre, { enable: false });
    ...
    describe("Plugin Integration", () => {
        it("should provide working registry methods", () => {
            ...
            expect(registry.getByAddress("0x0000000000000000000000000000000000000064")).to.not.be.undefined;
            expect(registry.getByAddress("0x000000000000000000000000000000000000006c")).to.not.be.undefined;
        });
        it("should handle precompile calls through registry", async () => {
            ...
            const result = await registry.handleCall("0x0000000000000000000000000000000000000064", // ArbSys address
            calldata, context);
